...
public class main_app extends javax.swing.JFrame {
    ...
    final Logger logger = LoggerFactory.getLogger(main_app.class);

    private static void init_sim_parameters() {
        s = new scheduler(600000, false);
    }

    private String conf_file_path="src/reti_tlc_gruppo_0/conf.xml";
...
//        File conf_file = new File(jTextField1.getText());
        File conf_file = new File(conf_file_path);
        ...
        if (conf_file.exists()) {
           ...
           // System.out.println("File non esistente");
            logger.error("file non esistente");
...
<canale id="0" tipo="WIRED" capacita="5000000" dim_pacchetto = "1526" tempo_propagazione = "10"></canale> 
                ...
                for (Object listElement2 : listElement1) {
                   ...
                   // System.out.println("tipo: "
                    ...
                    logger.info("tipo : {}",((Element) listElement2).getAttributeValue("tipo"));
                ...
                //    c.setIncrementalId(incrementalId);
                  //  incrementalId++;
...
//                LinkLayer ll = new LinkLayer(s, 5.0);
                LinkLayerStopAndWait ll = new LinkLayerStopAndWait(s, 5.0,id);
                s.addFineFilterObject(ll);
                
//                netLayerLinkState nl = new netLayerLinkState(s, 5.0, grafo);
                  NetworkLayer nl = new NetworkLayer(s, 5.0, grafo);
//                waveNetLayer nl = new waveNetLayer(s,5.0,grafo);
...
//                List listElement1 = node.getChildren("canale");
                ...
                for (Object interfaces_list : listElement1) {
                    ...
                    for (Object obj_interfaccia : intertace_list) {
                       // System.out.println("idinterfaccia:" + ((Element) obj_interfaccia).getAttributeValue("id"));
                        logger.info("id interfaccia : {}",((Element) obj_interfaccia).getAttributeValue("id"));
              ...
              //  System.out.println("Creato nodo_host con id: " + nh.getId() + " e gateway:" + nh.getGTW());
                logger.info("Creato nodo_host con id: {} e getaway: {}",nh.getId(),nh.getGTW());
            ...
            for (Object routers_list : listElement) {
                ...
                physicalLayer pl = new physicalLayer(s, 0.0);
//                LinkLayer ll = new LinkLayer(s, 5.0);
                LinkLayerStopAndWait ll = new LinkLayerStopAndWait(s, 5.0,node_id);
                s.addFineFilterObject(ll);
...
//TRASP                
               ...
               // System.out.println("Ho aggiunto un " + nr.getTipo() + " con id..:" + nr.getId());
                logger.info("ho aggiunto un {} con id {}",nr.getTipo(),nr.getId());
...
//Popolazione iniziale topologia                        
                ...
                List tabellaList = ((Element) routers_list).getChildren("routing_table");
                
                for (Object tabella : tabellaList) {
                	tabellaRouting tr = new tabellaRouting();
                	List rowList = ((Element) tabella).getChildren("table_row");
                	for (Object row : rowList) {
                		int dest = Integer.valueOf(((Element) row).getAttributeValue("dest"));
                		int next = Integer.valueOf(((Element) row).getAttributeValue("next"));
                		double hop = Double.valueOf(((Element) row).getAttributeValue("hop"));
                		tr.addEntry(dest, next, hop);
                	}
                	nl.setTR(tr);
                }
