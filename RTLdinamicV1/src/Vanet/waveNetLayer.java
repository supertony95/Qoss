...
public class waveNetLayer extends NetworkLayer {
	...
	final double wave_length = 0.051; // Lunghezza d'onda alla frequenza 5.0 GHz
										// IEEE802.11
	...
	public void Handler(Messaggi m) {
		...
		if (m.getTipo_Messaggio().equals(DISCOVER_NEIGHBOURS)) {
			...
			if (distance < NEIGHBOUR_THRESHOLD) {
				...
				// System.out.println("Livello PwTX :" + pw_tx + " PwRx:" +
				// pw_rx);
				// ptx/prx ratio
				...
				pw_rx = Math.pow(10, pw_rx / 10); // Calcolo potenza in mW da
													// dBm
				averagePwRx += pw_rx;
				counterAvgPwRx++;
				...
				// network.addEdge(edgeLabel, "" + ((Nodo) nodo).getId(), "" +
				// ns.getId());
				// network.getEdge(edgeLabel).setAttribute("ui.style",
				// "fill-color: blue;");
				// } else {
				// appCost = (Double)
				// network.getEdge(edgeLabel).getAttribute("cost");
				// }
				...
				for (HashMap.Entry<String, RoutingRow> e : ((HashMap<String, RoutingRow>) m.getData()).entrySet()) {
					...
					if (routes.containsKey("" + e.getKey())) {
						...
						if (costo_next_hop_dest + costo_sorg_next_hop < currentCost) {
							RoutingRow r1 = new RoutingRow(destId, nextHop, costo_next_hop_dest + costo_sorg_next_hop);

							routes.put(e.getKey(), r1);
							// if (network.getNode("" + r.getNodoDestinazione())
							// == null) {
							// network.addNode("" + r1.getNodoDestinazione());
							// network.getNode("" +
							// r1.getNodoDestinazione()).addAttribute("label",
							// "" + r1.getNodoDestinazione());
							// network.getNode("" +
							// r1.getNodoDestinazione()).addAttribute("ui.style",
							// "fill-color: yellow; size: 25px,25px;");
							//
							// network.addEdge(r1.getNextHop() + "-" +
							// r1.getNodoDestinazione(), "" + r1.getNextHop(),
							// "" + r1.getNodoDestinazione());
							// network.getEdge(edgeLabel).setAttribute("ui.style",
							// "fill-color: red;");
							// network.getEdge(edgeLabel).addAttribute("label",
							// "rosso");
							//
							// } else {
							// for (Edge edge : network.getEdgeSet()) {
							// if (edge.getNode1().toString().equals("" +
							// r1.getNodoDestinazione())) {
							// try {
							// network.removeEdge(edge);
							// String newEdgeLabel = r1.getNextHop() + "-" +
							// r1.getNodoDestinazione();
				...
				// System.out.println(s.orologio.getCurrent_Time() + " Inserisco
				// un nuovo vicino " + ((NodoMacchina) nodo).getId() + "->" +
				// ((NodoMacchina) m.getNodoSorgente()).getId() + "(" + cost +
				// ")");
				// System.out.println(routes);
			} else {
				// Se i nodi sono troppo distanti dobbiamo rimuovere tutte le
				// entries dove
			...
			// network.getNode("" + ((Nodo)
			// this.nodo).getId()).addAttribute("ui.style", "fill-color: red;
			// size: 25px,25px;");
			// for (HashMap.Entry<String, RoutingRow> e : routes.entrySet()) {
			...
			// network.getNode(node1).addAttribute("ui.style", "fill-color:
			// blue; size: 25px,25px;");
			// }
			// if (network.getNode(node2) == null) {
			// network.addNode(node2);
			// network.getNode(node2).addAttribute("label", node2);
			// network.getNode(node2).addAttribute("ui.style", "fill-color:
			// blue; size: 25px,25px;");
			// }
			//
			...
			// network.addEdge(edgeLabel, "" + ((Nodo) this.nodo).getId(),
			// node2);
			// }
			// else
			// {
			...
			// FileWriter("output/routes_" + ((NodoMacchina) nodo).getId() +
			// ".txt", true));
			// bw.write("\nTempo:" + s.orologio.getCurrent_Time() + "\n");
			// bw.write("Destinazione\t NEXTHOP\t Costo\n");
			// for (HashMap.Entry<String, RoutingRow> e : routes.entrySet()) {
			// RoutingRow r = e.getValue();
			// bw.write(r.getNodoDestinazione() + "\t" + r.getNextHop() + "\t" +
			// r.getCosto() + "\n");
			// }
			// bw.write("=========================\n");
			// bw.close();
			// } catch (IOException ex) {
			// Logger.getLogger(waveNetLayer.class.getName()).log(Level.SEVERE,
			// null, ex);
			// }
			updateNetwork();
		} else if (m.getTipo_Messaggio().equals(SEND_DISCOVER_NEIGHBOURS)) {

			if (!(nodo instanceof NodoSemaforo)) {
				...
				for (Nodo ns : nodes) {
					if (ns.getId() != n.getId()) {
						Messaggi m1 = new Messaggi(DISCOVER_NEIGHBOURS, this, this.linkLayer, ns,
	...
	private void gestisciPacchettoLivelloTransport(Messaggi m) {
		// Bisogna trovare il percorso per raggiungere la destinazione
		// utilizzando
		...
		if (routes.containsKey("" + destId)) {
			...
			for (Node node : mst.getPathNodes(network.getNode("" + destId))) {
				// node.addAttribute("ui.style", "fill-color: blue; size:
				// 25px,25px;");
	...
	private void updateNetwork() {
		...
		// try {
		// BufferedWriter bw = new BufferedWriter(new
		// FileWriter("output/routes_" + ((NodoMacchina) nodo).getId() + ".txt",
		// true));
		// bw.write("\nTempo:" + s.orologio.getCurrent_Time() + "\n");
		// bw.write("Destinazione\t NEXTHOP\t Costo\n");
		// for (HashMap.Entry<String, RoutingRow> e : routes.entrySet()) {
