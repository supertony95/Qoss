...
public class tcpTransportLayer extends TransportLayer {
    ...
    public void Handler(Messaggi m) {
        Applicazione a;
        Nodo n = (Nodo)this.nodo;
       // System.out.println("nodo "+n.getId());
        try{
        if (m.getTipo_Messaggio().toLowerCase().equals(SVUOTA_CODA)) {

            double tempo = 0;
            a = super.getApplication(m.getApplication_port());
            // System.out.println("application port"+a.getPort());
            if (a!=null && a.getPort() == m.getApplication_port()) {
            ...
            if (isAvailable(m.getApplication_port())) {
                ...
                if (m.saliPilaProtocollare == false) {
                    ...
                    if (sessionActive(m) == ACCEPTED) {
                        ...
                        buffer.add(m);
                         a = getApplication(m.getApplication_port());
            ...
            if (((Boolean) m.getData()) == true) {
                ...
                setAppicationReceiveWin(m);
                 ...
                 a = this.getApplication(m.getApplication_port());
        ...
        e.printStackTrace();
        System.out.println("simulation time "+s.orologio.getCurrent_Time());
    }
    }
