...
public class TransportLayer extends Entita {
    ...
    public TransportLayer(schedulerDV s, double tempo_di_processamento) {
...
//Gestione tipo UDP ; Per la gestione TCP implementare meccanismo di Congestion Avoidance            
            ...
            if (m.saliPilaProtocollare == false) {
                if (sessionActive(m) == ACCEPTED && buffer.isEmpty()) {
                    m.setSorgente(this);
                    m.setDestinazione(this.networkLayer);
                    ...
                    m.setNodoSorgente(nodo);
                    s.insertMessage(m);
                } else if (sessionActive(m) == WAITING || !buffer.isEmpty()) {
                    m.shifta(1000);
                    s.insertMessage(m);
                } else {
                    this.packet_refused++;
                ...
                System.out.println("Il numero massimo di hop : "+(m.numberHop-1));
                System.out.println("L'applicazione ha terminato l'invio e ho ricevuto dal nodo :" + ((NodoDV) m.getNodoSorgente()).getId() + "il seguente messaggio");
                Applicazione a = null;
                for (Object obj : apps) {
                    a = (Applicazione) obj;
